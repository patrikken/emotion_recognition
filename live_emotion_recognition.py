...
model = torch.load('emotion_recognition/Trained_Model')
model.eval()

labels = ['Happyness', 'Neutral', 'Sadness',
          'Surprise']  # Fear, Anger, Contempt
colors = [(0, 255, 0), (240, 240, 240), (0, 0, 255),  (255, 0, 255)]
...
while True:
    ...
    for (x, y, w, h) in face_rect:
        ...
        cv.putText(img, f'Emotion: {labels[label]}({prob.item():.2F})', (x, y-10),
                   cv.FONT_HERSHEY_SIMPLEX, .7, colors[label])
        #cv.imshow('RESIZED', resized_tensor[0].permute(1, 2, 0).numpy())
    cTime = time.time()
    fps = 1 / (cTime - pTime)
    pTime = cTime
